...
# ── שעת פרסום משתנה ─────────────────────────────────────────────────────────
# פרסום בשעה קבועה בול, יום אחר יום, הוא חתימה של אוטומציה. ה-Scheduler יורה
# בכל שעה עגולה בין 08:00 ל-19:00 (שעון ישראל), והשער הזה נותן לשעה אחת בלבד
# לעבור. השעה נגזרת מהתאריך, כך שהיא יציבה בתוך היום ומשתנה בין ימים — בלי
# קובץ מצב, בלי הרשאה נוספת, ובלי מסלול שיכול להיכשל ולהשתיק את המכונה.
#
# היושב לפני npm ci בכוונה: 11 מתוך 12 ההרצות היומיות יוצאות תוך שניות.
#
# שעת הפרסום הנראית מקבלת פיזור נוסף בחינם · הריצה עצמה נמשכת דקות, וזמן
# הקומיט הוא סופה ולא תחילתה.
#
# FORCE_RUN=1 עוקף · להרצה ידנית.
if [ "${FORCE_RUN:-0}" != "1" ]; then
  DAY=$(TZ=Asia/Jerusalem date +%Y-%m-%d)
  HOUR=$(TZ=Asia/Jerusalem date +%-H)
  SLOT=$(( 8 + $(printf '%s' "$DAY" | cksum | cut -d' ' -f1) % 12 ))
  if [ "$HOUR" != "$SLOT" ]; then
    echo "שעת הפרסום היום: ${SLOT}:00 · השעה עכשיו ${HOUR}:00 — יוצא."
    exit 0
  fi
  echo "שעת הפרסום היום: ${SLOT}:00 · מתחיל."
fi
