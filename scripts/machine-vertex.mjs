...
// ── סולם-מודלים לסבבי-התיקון (בקשת ליאור): סבב 1 = Flash (מהיר/זול) · סבב 2+ = Pro (החזק). ──
// כל סבב מטפס. flash זמין רק ב-region 'global'; pro ב-us-central1. הרעיון: ה-QA לא "דוחה" —
// הוא נכשל-על-בדיקה → מתקן (במודל חזק יותר בכל סבב) → נבדק שוב → ומשגר. רק אם אחרי כל הסבבים
// עדיין נותרה בעיה קשה (מבנה/ייחוס) → גניזה + התראת-טלגרם, לעולם לא פרסום שבור.
const FIX_MODELS = (process.env.FIX_MODELS || 'gemini-2.5-flash,gemini-2.5-pro,gemini-2.5-pro')
...
if (isMain) try {
  ...
  while (coreQaRan && (issues.length || suggestedSlug) && round < MAX_FIX_ROUNDS) {
    ...
    console.error(`→ fix round ${round}/${MAX_FIX_ROUNDS}${aggressive ? ' [aggressive]' : ''} · [${fixModel}] · ${issues.length} issue(s)${suggestedSlug ? ' +slug' : ''}`)
    const fx = parseArticle((await callGemini(fixPrompt(`SLUG: ${suggestedSlug || slug}\n\n${md}`, issues, { suggestedSlug, aggressive }), { maxTokens: 16000, temperature: 0.3, model: fixModel, region: fixRegion })).text)
  ...
  // ── SHIP or SHELVE · if a HARD issue (structure/attribution) survived every escalating fix
  //    round, the article is not publishable. Per spec: never ship broken and never silently
  //    reject — park it as a draft (excluded from build/index/sitemap) and alert on Telegram. ──
  const shelved = hasHardIssue(residual)
  if (shelved) console.error(`⛔ shelving after ${round} escalating round(s) — hard issue survived: ${residual.filter((i) => HARD_ISSUE_RE.test(i)).slice(0, 3).join(' · ')}`)
  const publishNow = PUBLISH_NOW && !shelved
  ...
  markDone(planKeyword)
  console.error(`${shelved ? '⛔ shelved' : '✓ wrote'} ${slug}.md  (draft:${!publishNow}, qa:${qaNote}, ${round} fix round(s), ${CALLS} model calls)`)
  ...
  // מאמר גנוז לא מקבל קאבר (בזבוז קריאת-Imagen על משהו שלא עולה).
  let coverPath = null
  if (!shelved) try {
  ...
  result({
    status: shelved ? 'shelved' : (publishNow ? 'published' : 'banked'), cluster: cat.slug, slug, title, description,
    ...
    shelveReason: shelved ? `נגנז אחרי ${round} סבבי-תיקון · בעיה קשה שלא נסגרה: ${residual.filter((i) => HARD_ISSUE_RE.test(i)).slice(0, 3).join(' · ')}` : '',
    qaWarning: !shelved && residual.length ? `שוגר עם ${residual.length} הערה שלא נסגרה (${qaNote})` : '',
  })
} catch (e) {
